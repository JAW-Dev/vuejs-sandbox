<template>
  <p
    ...
    v-if="options.author.showAuthor && article[options.author.source]">
    <span
      :id="options.author.label.ID"
      :class="options.author.label.classes"
      v-bind="options.author.label.attributes"
      v-if="options.author.label.showLabel">{{ options.author.label.labelText }}</span>
    <card-link
      ...
      :renderLink="renderElement(article[options.author.link.source] && options.author.link.showLink)">
      <span
        slot="content"
        :id="options.author.text.ID"
        :class="options.author.text.classes"
        v-bind="options.author.text.attributes">{{ article[options.author.source] }}</span>
    </card-link>
  </p>
</template>
